...
public class TwitterAuth 
{
	...
	public String get(@PathParam("id") Long id,@QueryParam("oauth_verifier") String code)
	{
		...
		{			
			...
			file.delete();
			inStream.close();
			AccessToken accessToken=twitter.getOAuthAccessToken(requestToken,code);
			System.out.println(accessToken.getToken());
			System.out.println(accessToken.getTokenSecret());
			SocialNetworking sn=SocialNetworking.all().filter("name=?", "twitter").fetchOne();
			stat=accessToken.getToken()+":"+accessToken.getTokenSecret()+":"+accessToken.getScreenName();
			if(!accessToken.getToken().isEmpty())
			{
				PersonalCredential personalCredential=new PersonalCredential();
				personalCredential.setSnUserName("@"+accessToken.getScreenName());
